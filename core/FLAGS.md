...
| Flag | 别名 | 描述 | Token 预算 |
|------|------|------|------------|
| `--think` | `--深度`, `-t` | 扩展思考，考虑多个方案 | 2x 标准 |
| `--ultrathink` | `--好好思考`, `-ut` | 最大思考预算，适合复杂决策 | 5x 标准 |
| `--quick` | `--快速`, `-q` | 最小思考，快速执行 | 0.5x 标准 |
...
| 关键词 | 自动 Flag |
|--------|-----------|
| `design`, `architect`, `设计`, `架构` | `--think` |
| `fix`, `patch`, `修复`, `修补` | `--quick` |
| `好好想想`, `认真考虑`, `think through` | `--ultrathink` |
| `快速`, `赶紧`, `urgent`, `asap` | `--quick` |
| `全力`, `parallel`, `并行` | `--parallel` |
